...
public class PlayerMovement : PlayerBase
{
    ...
    {
        float groundY = collision.gameObject.GetComponent<BoxCollider2D>().size.y * collision.transform.lossyScale.y / 2f + collision.transform.position.y;

        if (dont) // this bool exists for when you jump while you're sprinting, because the collider is switched then
        {
            dont = false;
        }
        else if (collision.gameObject.tag == "Ground" && (!grounded || !test) && groundY < transform.position.y - (0.13f * 9))
        {
            grounded = true;
            test = true;
